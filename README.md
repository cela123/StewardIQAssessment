## Project Overview
This project demonstrates a full-stack application with a React + Vite frontend and a .NET backend API. It showcases the ability to create modular, maintainable components with TypeScript and Material-UI (MUI), as well as handle API integration and filtering.

### Technologies Used
Frontend: React, TypeScript, Vite, Material-UI (MUI) 
Backend: .NET 8 / ASP.NET Core Web API Other: Node.js, npm

### Getting Started
#### Frontend
Navigate to the frontend directory: (cd StewardIQAssessment/frontend)
Install dependencies: npm install
Start the development server: npm run dev
The frontend will run at: http://localhost:5173.

#### Backend
Open the solution in your IDE (e.g., Visual Studio).
Set StewardIQAssessment.Api as the startup project.
Run the project (F5 or Start Debugging).
The backend API will run at: http://localhost:7091.

### AI Assistance
During this assessment, AI was used in the following areas:

Troubleshooting and resolving Vite proxy issues
Ensuring the frontend correctly communicates with the backend API.
Creating structured components with TypeScript and MUI.
